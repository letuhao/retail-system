...
type ProScreenProps = Partial<NativeStackScreenProps<RootStackParamList, 'ProScreen'>>;

class ProScreen extends React.Component<ProScreenProps> {
    render() {
                ...
                <Block flex>
                    ...
                    <Block space="between" style={styles.padded}>
                        <Block>
                            ...
                            <Block row style={{ marginTop: theme.SIZES?.BASE ?? 0 * 1.5, marginBottom: theme.SIZES?.BASE ?? 0 * 4 }}>
                                <Image
                                    source={require('../assets/images/ios.png')}
                                    style={{ height: 38, width: 82, marginRight: theme.SIZES?.BASE ?? 0 * 1.5 }}
                                />
                                <Image
...
const styles = StyleSheet.create({
    ...
    padded: {
        ...
        zIndex: 3,
        position: 'absolute',
        bottom: Platform.OS === 'android' ? theme.SIZES?.BASE ?? 0 * 2 : theme.SIZES?.BASE ?? 0 * 3,
    },
    button: {
